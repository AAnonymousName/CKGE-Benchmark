...
def tp_new_ent(tp, ratio, entities_count=8812,top_n=2600, sample_num=645):
    ent = set()
    for line in tp:
    ...
    ratio_int = int(ratio.strip().split(':')[1])
    tp_random = random.sample(tp_remain, 62023 * ratio_int - len(keys_list))
    ...
    all_rel = set()
    with open("FB15k-237/relations.dict", 'r') as f:
        for line in f:
    ...
    all_ent = set()
    with open("FB15k-237/entities.dict", 'r') as f:
        for line in f:
...
def split_data(s0,s1,data_name):
    ...
    with open('../data/PS-CKGE/{}/0/train.txt'.format(data_name), 'w') as f:
        for tp in train:
            f.write(tp + '\n')
    with open('../data/PS-CKGE/{}/0/valid.txt'.format(data_name), 'w') as f:
        for tp in valid:
            f.write(tp + '\n')
    with open('../data/PS-CKGE/{}/0/test.txt'.format(data_name), 'w') as f:
        for tp in test:
            f.write(tp + '\n')

    ent = set()
    with open('../data/PS-CKGE/{}/0/train.txt'.format(data_name), 'r') as f:
        for line in f:
    ...
    with open('../data/PS-CKGE/{}/1/train.txt'.format(data_name), 'w') as f:
        for tp in train:
            f.write(tp + '\n')
    with open('../data/PS-CKGE/{}/1/valid.txt'.format(data_name), 'w') as f:
        for tp in valid:
            f.write(tp + '\n')
    with open('../data/PS-CKGE/{}/1/test.txt'.format(data_name), 'w') as f:
        for tp in test:
            f.write(tp + '\n')
...
def main():
    ...
    parser.add_argument('--ratio', type=str, help="Data ratio")
